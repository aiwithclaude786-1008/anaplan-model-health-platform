...
#
# Every detect() below is vectorized -- it takes the whole feats
# DataFrame and returns a boolean Series in one pandas expression,
# rather than being called once per row. This is what keeps a
# large export's rule pass fast (a handful of column comparisons
# per rule instead of rows x rules Python-level calls).
# ============================================================
...
def build_formula_rules(t: RuleThresholds) -> List[Rule]:
    return [
        Rule(
            ...
            detect=lambda f: f["count_lookup"] > t.lookup_multi_threshold,
            severity="high",
        ...
        Rule(
            ...
            detect=lambda f: f["count_if"] >= t.nested_if_high,
            severity="high",
        ...
        Rule(
            ...
            description="Keep IF logic shallow -- long chains should become a mapping module.",
            detect=lambda f: (f["count_if"] >= t.nested_if_med) & (f["count_if"] < t.nested_if_high),
            severity="medium",
            recommendation="Replace IF chains with a mapping module.",
            confidence=CONFIDENCE_MEASURED, affects_performance=True,
        ),
        Rule(
            ...
            detect=lambda f: f["map_has_lookup_sum"],
            severity="high",
        ...
        Rule(
            ...
            detect=lambda f: f["map_has_lookup_select"],
            severity="high",
        ...
        Rule(
            ...
            detect=lambda f: f["map_has_select_sum"],
            severity="high",
        ...
        Rule(
            ...
            detect=lambda f: f["func_density_count"] >= t.daisy_chain_threshold,
            severity="high",
        ...
        Rule(
            ...
            detect=lambda f: f["has_post_lookup_nested"],
            severity="high",
        ...
        Rule(
            ...
            detect=lambda f: f["has_post_sum_nested"],
            severity="high",
        ...
        Rule(
            ...
            detect=lambda f: f["has_post_inside_if"],
            severity="high",
        ...
        Rule(
            ...
            detect=lambda f: f["has_timesum"] & ~f["has_start_or_end"],
            severity="high",
        ...
        Rule(
            ...
            detect=lambda f: f["has_select_hardcoded"],
            severity="medium",
        ...
        Rule(
            ...
            detect=lambda f: f["formula_length"] >= t.formula_length_high,
            severity="medium",
